/* ************************************************************************** */
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   utils.c                                            :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: ychoucho <ychoucho@student.1337.ma>        +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2026/07/27 20:04:49 by ychoucho          #+#    #+#             */
/*   Updated: 2026/07/27 21:42:51 by ychoucho         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "codexion.h"

void	swap(t_coder_info *a, t_coder_info *b)
{
	t_coder_info	temp;

	temp = *a;
	*a = *b;
	*b = temp;
}

int	sim_status(t_thread_data *coder)
{
	int	status;

	pthread_mutex_lock(&coder->sim->stop_mutex);
	status = coder->sim->sim_status;
	pthread_mutex_unlock(&coder->sim->stop_mutex);
	return (status);
}

long	get_time(struct timeval start_time)
{
	long			milliseconds;
	struct timeval	current_time;

	gettimeofday(&current_time, NULL);
	milliseconds = (current_time.tv_sec - start_time.tv_sec) * 1000
		+ (current_time.tv_usec - start_time.tv_usec) / 1000;
	return (milliseconds);
}

void	safe_print(t_thread_data *data, char *text)
{
	pthread_mutex_lock(&data->sim->print_mutex);
	if (sim_status(data) || strcmp(text, "%lu %d burned out\n") == 0)
		printf(text, get_time(data->sim->start_time), data->info->thread_id);
	pthread_mutex_unlock(&data->sim->print_mutex);
}

int	safe_sleep(t_thread_data *coder, long ms)
{
	long	start;

	start = get_time(coder->sim->start_time);
	while (sim_status(coder))
	{
		if (get_time(coder->sim->start_time) - start >= ms)
			break ;
		usleep(500);
	}
	return (sim_status(coder));
}
